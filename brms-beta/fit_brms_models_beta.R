...
# filters: only females, no NAs, non-kin only
d4 <- d3 %>% 
  filter(a.sex == "F") %>% 
  filter(r.sex == "F") %>% 
...
# fit brms model of effect of sharing on vocal similarity
fit_fs <-
  brm(sim ~ 
        scale(sharing) + 
        scale(kinship) +
...
# fit brms model of effect of grooming on vocal similarity
fit_g <-
  brm(sim ~ 
        scale(grooming) + 
        scale(kinship) +
