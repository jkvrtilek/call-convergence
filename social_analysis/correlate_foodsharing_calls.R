# predict vocal similarity from food sharing in 2015 bats
# updated 10 April 2024 to include food sharing from 2017
...
means <- 
  ...
  mutate(categories = case_when(
    foodsharing == 0 ~ "no food sharing\nobserved",
    foodsharing >= 0 ~ "food sharing\nobserved")) %>% 
  filter(!is.na(dist)) %>% 
...
p <- 
  ...
  xlab("")+
  ggtitle("nonkin call similarity by food-sharing relationship")+
  theme_bw()
p
