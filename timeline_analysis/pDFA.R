...
# note: scs is only bat in group 3
...
#### 3
#### pre-introduction period Tole bats who are present at post-intro period 2
#### vs post-intro period 2 Las Pavas

xdata <- rbind(tole_pre_3, lp_3)
test_fac <- as.factor(as.character(xdata$group))
contr_fac <- as.factor(as.character(xdata$individual))
variables <- paste(colnames(specan2)[3:ncol(specan2)], collapse = "+")
...
# adapted from https://www.r-bloggers.com/2014/01/computing-and-visualizing-lda-in-r/
predictions <- predict(dfa)
dataset3 = data.frame(bat = xdata[,"individual"], group = xdata[,"group"], lda = predictions$x)

p3 <- ggplot(data = dataset3, aes(x = lda.LD1, y = lda.LD2, color = group)) + 
  geom_point(size = 0.1, alpha = 0.3) +
  labs(x = "LD1", y = "LD2", color = "") +
  scale_color_manual(values = c("#0f2d59","#3eb7c7")) +
  theme_minimal() +
...
#### 4
#### post-intro period 2 Tole bats
#### vs post-intro period 2 Las Pavas

xdata <- rbind(tole_3, lp_3)
test_fac <- as.factor(as.character(xdata$group))
contr_fac <- as.factor(as.character(xdata$individual))
variables <- paste(colnames(specan2)[3:ncol(specan2)], collapse = "+")
...
# adapted from https://www.r-bloggers.com/2014/01/computing-and-visualizing-lda-in-r/
predictions <- predict(dfa)
dataset4 = data.frame(bat = xdata[,"individual"], group = xdata[,"group"], lda = predictions$x)

p4 <- ggplot(data = dataset4, aes(x = lda.LD1, y = lda.LD2, color = group)) + 
  geom_point(size = 0.1, alpha = 0.3) +
  labs(x = "LD1", y = "LD2", color = "") +
  scale_color_manual(values = c("#0f2d59","#3eb7c7")) +
  theme_minimal() +
...
subtitle1 <- textGrob("Post-introduction period 1 Las Pavas bats vs. pre-introduction (left) and post-introduction period 1 (right) Tole bats.")
subtitle2 <- textGrob("Post-introduction period 2 Las Pavas bats vs. pre-introduction (left) and post-introduction period 2 (right) Tole bats.")
lay <- rbind(c(1,2),
             c(1,2),
             c(1,2),
             c(1,2),
             ...
             c(4,5),
             c(6,6))
grid_arrange_shared_legend(p1,p2,subtitle1,p3,p4,subtitle2,ncol=2,nrow=5,
